...
import { preparePlayer } from "@/tests/browser/voice-mocks";
...
const emit = async (page: Page, event: string) => page.locator("mux-player").evaluate((player, name) => player.dispatchEvent(new Event(name)), event);
